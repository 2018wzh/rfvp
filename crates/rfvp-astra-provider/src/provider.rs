...
impl LegacyRuntimeProvider for FvpRuntimeProvider {
    ...
    fn probe(
            ...
            self.case_for_mount(&request.root_mount_id)
        {
            ...
            for uri in &request.candidate_uris {
                ...
                let bytes =
                    host.vfs
                        .read_file(&request.root_mount_id, uri, MAX_FILE_BYTES as u64)?;
    ...
    fn step(
        ...
        let text_operations = session
            .runtime
            .take_text_events()
            .map_err(|error| invalid("ASTRA_FVP_TEXT_EVENTS", error.to_string()))?;
        ...
        tracing::debug!(
            ...
            text_operation_count = text_operations.len(),
            hosted_log_count = delta.logs.len(),
            hosted_log_dropped_count = delta.log_dropped_count
        );
        if delta.log_dropped_count != 0 {
            tracing::warn!(
                event = "astra.emu.fvp.hosted_log_dropped",
                fixed_step = input.tick_index,
                dropped_count = delta.log_dropped_count,
                "structured evidence logs were bounded and dropped"
            );
            diagnostics.push(LegacyDiagnostic {
                code: "ASTRA_FVP_LOG_DROPPED".into(),
                severity: "warn".into(),
                subject: "rfvp.hosted.log".into(),
                message: "structured evidence logs were bounded and dropped".into(),
            });
        }
