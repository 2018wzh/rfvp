...
//!
//! This is the concrete v9 execution boundary used by the dynamic provider:
//! the RFVP core and its non-`Send` VFS cursors remain on one worker while the
//! caller exchanges typed audio/video output and a Host-owned surface lease.
...
use rfvp_hosted::{
    hosted::{
        ...
        HostedStepInput, HostedTextEvent, HostedTraceProfile,
    },
    script::parser::Nls,
...
impl HostedFvpSession {
    ...
    pub fn take_text_events(&self) -> Result<Vec<HostedTextEvent>, HostedRuntimeError> {
        self.worker
            .execute_result(|state| Ok::<_, HostedRuntimeError>(state.core.take_text_events()))?
    }
...
mod tests {
    ...
    fn case_lifecycle_renders_directly_into_the_supplied_allocation() {
        ...
        let pixels: astra_byte_source::OwnedWritableByteBuffer = vec![0; 1024 * 768 * 4].into();
        let original_allocation = pixels.as_slice().as_ptr();
        let pixels = session
            .render_surface(1024, 768, PixelFormat::Rgba8, pixels)
            .expect("direct surface must render");
        assert_eq!(pixels.len(), 1024 * 768 * 4);
        assert_eq!(pixels.as_slice().as_ptr(), original_allocation);
        assert!(!session
            .quit_requested()
